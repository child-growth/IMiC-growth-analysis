...
r2weightFunc <- function(X, Y) {
  ...
  r2.out1 <- r2_optWeight(out1, Y = Yscale, X = X)
  
  # Make a dataframe
  results <- data.frame(matrix(ncol = 4))
  names(results) = c("est", "CI.l", "CI.h", "p.value")
  
  for (i in 1:ncol(X)) {
  # Measure importance of every feature
  out2 <- optWeight(Y = Yscale, X = X[, -i], SL.library = c("SL.glm","SL.mean"))#,"SL.step"))
  
  # Get R-squared for combined outcome excluding each feature one at a time
  r2.out2 <- r2_optWeight(out2, Y = Yscale, X = X[, -i])
  ...
  results[i, 1] = outDiff.r2 $ diff[, 1]
  results[i, 2] = outDiff.r2 $ diff[, 2]
  results[i, 3] = outDiff.r2 $ diff[, 3]
  results[i, 4] = outDiff.r2 $ diff[, 4]
  }
