...
# Network analysis: first try based on correlation
```{r}
# Edge list
EL <- corMatrix4
...
# Network analysis: 2nd try based on distance matrix
```{r}
...
# Transpose the dataset so that each row is an HMO
transpose <- t(hmoScaled)

distMatrix <- as.matrix(dist(transpose))

g <- graph_from_adjacency_matrix(distMatrix, mode = "undirected")
