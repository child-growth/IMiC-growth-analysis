...
# Implement CVtreeMLE: text from the package description
```{r}
...
#write.csv(mixture_results, file = "/data/imic/results/CVtreeMLE.csv")

mixture_results %>%
  dplyr::filter(Proportion_Folds == 0.4) %>%
...
marginal_results %>%
  dplyr::filter(`Proportion in Fold` == 0.2) %>%
