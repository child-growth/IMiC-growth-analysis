...
```{r}
simulateY <- function(data) {
  mean1 <- mean(data $ haz_base)
  sd1 <- sd(data $ haz_base)

  # Generate new data from this distribution
  data $ y1Simulated <- rnorm(n = nrow(data), mean = mean1, sd = sd1)
  data $ y1Simulated <- as.numeric(format(round(y1Simulated, 2), nsmall = 2))
  
  mean2 <- mean(data $ haz_6m)
  sd2 <- sd(data $ haz_6m)

  # Generate new data from this distribution
  y2Simulated <- rnorm(n = nrow(data), mean = mean2, sd = sd2)
  y2Simulated <- as.numeric(format(round(y2Simulated, 2), nsmall = 2))
...
A1 = c("X2.FL", "X3FL", "DFLac", "X3.SL", "X6.SL", "LNT", "LNnT", "LNFP.I",
...
bioc $ haz_6m_simulated <- as.numeric(format(round(bioc $ haz_6m_simulated, 2), 
                                            nsmall = 2))
...
# Get mean and variance of the outcome
hist(hmo $ haz_m6)
mean <- mean(hmo $ haz_m6)
sd <- sd(hmo $ haz_m6)

# Generate new data from this distribution
hmo $ haz_6m_simulated <- rnorm(n = 147, mean = mean, sd = sd)
hmo $ haz_6m_simulated <- as.numeric(format(round(hmo $ haz_6m_simulated, 2), 
                                            nsmall = 2))
...
# Get mean and variance of the outcome
hist(bioc $ haz_m6)
mean <- mean(bioc $ haz_m6)
sd <- sd(bioc $ haz_m6)

# Generate new data from this distribution
bioc $ haz_6m_simulated <- rnorm(n = 147, mean = mean, sd = sd)
bioc $ haz_6m_simulated <- as.numeric(format(round(bioc $ haz_6m_simulated, 2), 
                                            nsmall = 2))
...
# Get mean and variance of the outcome
hist(metabol $ haz_m6)
mean <- mean(metabol $ haz_m6)
sd <- sd(metabol $ haz_m6)

# Generate new data from this distribution
metabol $ haz_6m_simulated <- rnorm(n = 147, mean = mean, sd = sd)
metabol $ haz_6m_simulated <- as.numeric(format(round(metabol $ haz_6m_simulated, 2), 
                                            nsmall = 2))
dput(names((metabol)))
