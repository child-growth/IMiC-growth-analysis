...
r2weightFunc <- function(X, Y) {
  ...
  return(list(results=results, out1=out1))
}


# Load datasets and prepare
# HMO Elicit
try(hmoE <- readRDS("/data/imic/data/raw_lab_data/elicit/merged_elicit/hmo.RDS"))
...
# Vitamin B Elicit
try(bvitE <- readRDS("/data/imic/data/raw_lab_data/elicit/merged_elicit/bvit.RDS"))
try(bvitE <- readRDS("C:/Users/andre/Downloads/bvit.RDS"))
...
try(write.csv(hmoEresult, file = "/data/imic/results/r2weight/hmoEr2weight.csv"))
saveRDS(hmoEresult, file=paste0(here::here(),"/results/hmoEr2weight.RDS"))

# Elicit Bvit
bvitResult <- r2weightFunc(X = XbvitE, Y = YbvitEscale)
bvitResult $ predictors = predictors

try(write.csv(bvitResult, file = "/data/imic/results/r2weight/bvitEr2weight.csv"))
saveRDS(bvitResult, file=paste0(here::here(),"/results/bvitEr2weight.RDS"))
...
out2 <- optWeight(Y = YbvitEscale, X = XbvitE[, 1:6], SL.library = c("SL.glm","SL.mean"))#,"SL.step"))
