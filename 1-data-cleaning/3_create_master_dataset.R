...
Zscores <- Zscores %>% 
  subset(., select = c(studyid, country, subjid, sex, agedays,waz, haz, whz, muaz, lencm, wtkg, hcircm , muaccm,  arm, brthweek, brthyr))
...
cov <- cov %>% subset(., select = -c(sex, muaz, lencm, wtkg, hcircm, muaccm, arm, brthweek,brthyr))
