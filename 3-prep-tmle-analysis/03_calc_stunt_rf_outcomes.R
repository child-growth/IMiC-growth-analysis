...
stunt_ci_6_18 = d6 %>% ungroup() %>% 
  ...
  mutate(agecat="6-18 months", minhaz=min(haz), ever_stunted=ifelse(minhaz< -2,1,0), ever_sstunted=ifelse(minhaz< (-3),1,0), Nobs=n()) %>% slice(1) %>%
  mutate(N=n()) %>%
  ungroup() %>%
  select(studyid,subjid, country,agedays,haz,  measid, agecat,minhaz, ever_stunted,ever_sstunted,Nobs, N)


#calculate any stunting from 0-18
stunt_ci_0_18 = d6 %>% ungroup() %>%
  filter(!is.na(agecat)) %>%
  group_by(studyid,country,subjid) %>%
  #create variable with minhaz by age category, cumulatively
  mutate(agecat="0-18 months", minhaz=min(haz), ever_stunted=ifelse(minhaz< -2,1,0), ever_sstunted=ifelse(minhaz< (-3),1,0), Nobs=n()) %>% slice(1) %>%
  mutate(N=n()) %>%
  ungroup() 
...
cuminc <- bind_rows(stunt_ci_0_6, stunt_ci_6_18, stunt_ci_0_18)
...
prev_stunt <- prev %>% droplevels()
table(prev_stunt$studyid, prev_stunt$agecat)

meanHAZ <- meanHAZ %>% droplevels()
cuminc_stunt <- cuminc %>% droplevels()


save(prev_stunt, file=paste0(ghapdata_dir,"st_prev_outcomes.RData"))
save(meanHAZ, file=paste0(ghapdata_dir,"st_meanZ_outcomes.RData"))
save(cuminc_stunt, file=paste0(ghapdata_dir,"st_cuminc_outcomes.rdata"))
