...
stunt_outcomes_E = calc_outcomes(data = d, calc_method = "REML", output_file_suffix = "")

stunt_outcomes_fe_E = calc_outcomes(data = d, calc_method = "FE", output_file_suffix = "_fe")
...
calc_outcomes = function(data, calc_method, output_file_suffix){
  dprev <<- calc.prev.agecat.vital(data)
  d3 <<- calc.ci.agecat.vital(data, range = 3, birth="yes")
  d6 <<- calc.ci.agecat.vital(data, range = 6, birth="yes")
  d3_birthstrat <<- calc.ci.agecat.vital(data, range = 3, birth="no")
  d6_birthstrat <<- calc.ci.agecat.vital(data, range = 6, birth="no")
  ...
  haz.vel.vital <- bind_rows(data.frame(cohort = "pooled", haz.data.vel$haz.res), haz.cohort.vel)
  
  saveRDS(haz.vel.vital, file = paste0(res_dir, "stunting/meanlaz_velocity_V", 
                                       calc_method, output_file_suffix, ".RDS"))
...
stunt_outcomes_V = calc_outcomes(data = d, calc_method = "REML", output_file_suffix = "")
stunt_outcomes_fe_V = calc_outcomes(data = d, calc_method = "FE", output_file_suffix = "_fe")
...
# Combine datasets
stunt_outcomes <- rbind(stunt_outcomes_V, stunt_outcomes_E)
stunt_outcomes_fe <- rbind(stunt_outcomes_fe_V, stunt_outcomes_fe_E)
...
# Also combine the haz.vel datasets
elicit <- readRDS(paste0(BV_dir,"/results/stunting/meanlaz_velocity_EREML.RDS"))
elicit_fe <- readRDS(paste0(BV_dir,"/results/stunting/meanlaz_velocity_EFE_fe.RDS"))
vital <- readRDS(paste0(BV_dir,"/results/stunting/meanlaz_velocity_VREML.RDS"))
vital_fe <- readRDS(paste0(BV_dir,"/results/stunting/meanlaz_velocity_VFE_fe.RDS"))

# Combine haz.vel datasets
haz.vel <- rbind(elicit, vital)
haz.vel.fe <- rbind(elicit_fe, vital_fe)

# Save
saveRDS(haz.vel, file = paste0(res_dir, "stunting/meanlaz_velocityREML.RDS"))
saveRDS(haz.vel.fe, file = paste0(res_dir, "stunting/meanlaz_velocityFE_fe.RDS"))
