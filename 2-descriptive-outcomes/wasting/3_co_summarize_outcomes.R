...
# m.prev.country <- d %>% filter(!is.na(muaz)) %>% group_by(country) %>% do(summary.prev.muaz(.)$m.prev.res)
# 
# m.prev.cohort <-
#   m.prev.data$m.prev.cohort %>% subset(., select = c(cohort, country, agecat, nmeas,  prev,  ci.lb,  ci.ub)) %>%
#   rename(est = prev,  lb = ci.lb,  ub = ci.ub)
# 
# muaz.prev <- bind_rows(
...
# prev.country <- d %>% filter(!is.na(muaz)) %>% group_by(country) %>% do(summary.prev.muaz(.)$prev.res)
# prev.cohort <-
#   m.prev.data$prev.cohort %>% subset(., select = c(cohort, country, agecat, nmeas,  prev,  ci.lb,  ci.ub)) %>%
#   rename(est = prev,  lb = ci.lb,  ub = ci.ub)
# 
